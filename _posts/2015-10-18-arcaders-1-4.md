...
```rust
view.render(&mut canvas, &mut events, elapsed);
```

It will also us to do some _caching_ and other performance optimizations when
we start working with external resources.

<!-- [TODO]: talk about type arguments 
RenderTarget: peut être Surface ou Window
Surface se comporte comme Renderer au niveau des lifetimes
-->
